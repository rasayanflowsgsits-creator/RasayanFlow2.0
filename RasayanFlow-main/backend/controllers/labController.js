...
/** Hard cap: each lab may have at most this many lab admins. */
const MAX_LAB_ADMINS = 2;

const ensureLabAdminsArray = (lab) => {
...
/**
 * Returns true if this user is already listed on lab.admins (re-assign / update is allowed).
 * Throws 400 if adding them would exceed MAX_LAB_ADMINS.
 */
const assertCanAddLabAdmin = (lab, adminUserId, res) => {
  const admins = ensureLabAdminsArray(lab);
  const adminIdStr = String(adminUserId);
  const alreadyOnLab = admins.some((id) => id && String(id) === adminIdStr);

  if (!alreadyOnLab && admins.length >= MAX_LAB_ADMINS) {
    res.status(400);
    throw new Error(`Lab already has ${MAX_LAB_ADMINS} admins`);
  }
...
const createLab = asyncHandler(async (req, res) => {
  ...
  // Link admin to lab admins array (new lab starts empty; still respect max-2 if array was seeded)
  if (provisionedAdmin) {
    const alreadyOnLab = assertCanAddLabAdmin(lab, provisionedAdmin._id, res);
    if (!alreadyOnLab) {
...
const assignAdmin = asyncHandler(async (req, res) => {
  ...
  // Enforce max admins BEFORE creating a brand-new user for a full lab
  // (so we do not leave orphan labAdmin accounts when assign would fail)
  if (!admin && normalizedEmail) {
    if (lab.admins.length >= MAX_LAB_ADMINS) {
      res.status(400);
      throw new Error(`Lab already has ${MAX_LAB_ADMINS} admins`);
    }
  } else if (admin) {
    assertCanAddLabAdmin(lab, admin._id, res);
  }
  ...
  // Re-check after resolve (covers race + already-on-lab updates)
  const alreadyOnLab = assertCanAddLabAdmin(lab, admin._id, res);
