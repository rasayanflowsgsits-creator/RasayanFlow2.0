...
export default function StoreHistory() {
  ...
  const headers = [
    ...
    { key: 'qtyBefore', label: 'Qty Before', render: (r) => `${safeRound(r.qtyBeforeUNT || 0).toLocaleString()} UNT (${safeRound(r.qtyBeforeBase || 0).toLocaleString()} ${r.baseUnit || 'ml'})` },
    { key: 'qtyRequested', label: 'Requested', render: (r) => `${safeRound(r.qtyRequestedBase || r.qtyRequested || 0).toLocaleString()} ${r.baseUnit || 'ml'}` },
    { key: 'qtyAfter', label: 'Qty After', render: (r) => `${safeRound(r.qtyAfterUNT || 0).toLocaleString()} UNT (${safeRound(r.qtyAfterBase || 0).toLocaleString()} ${r.baseUnit || 'ml'})` },
    { key: 'valueReleased', label: 'Value Released (₹)', render: (r) => formatPrice(safeRound(r.valueReleased || (r.totalValueBefore - r.totalValueAfter) || 0)) },
    { key: 'receiptNumber', label: 'Receipt No', render: (r) => r.receiptNumber || 'N/A' },
