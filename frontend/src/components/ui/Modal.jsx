...
export default function Modal({ open, title, onClose, children, panelClassName = 'max-w-xl' }) {
  if (!open) return null;
  ...
  return createPortal(
    <div className='fixed inset-0 z-50 overflow-y-auto bg-[#23281d]/35 p-3 backdrop-blur-sm sm:p-4'>
      <div className='flex min-h-full items-start justify-center py-3 sm:items-center sm:py-6'>
        <div className={`w-full ${panelClassName} overflow-hidden rounded-2xl border border-[#d9e1ca] bg-[#fffef8] shadow-soft dark:border-[#414a33] dark:bg-[#20251a] transform transition-all duration-300`}>
