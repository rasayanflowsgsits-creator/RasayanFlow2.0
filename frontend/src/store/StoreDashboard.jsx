...
export default function StoreDashboard() {
  ...
  return (
    ...
    >
      ...
      {/* STACKED HORIZONTAL FULL-WIDTH SECTIONS */}
      <div className="space-y-6">
        {/* SECTION 1: INVENTORY HEALTH & DEPLETION RISK MONITOR */}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#fafbf5] dark:bg-[#1a1d16] p-4 rounded-xl border border-[#e3e9d8] dark:border-[#2e3d19] space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3 text-center text-xs pt-1">
                <div className="rounded-lg bg-emerald-50 dark:bg-emerald-950/30 p-2.5 border border-emerald-200 dark:border-emerald-800/40">
                  <span className="block font-extrabold text-sm text-emerald-700 dark:text-emerald-400">{stockHealthStats.inStock}</span>
                  <span className="text-[11px] text-emerald-800/80 dark:text-emerald-300 font-medium">In Stock ({stockHealthStats.inStockPct}%)</span>
                </div>
                <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 p-2.5 border border-amber-200 dark:border-amber-800/40">
                  <span className="block font-extrabold text-sm text-amber-700 dark:text-amber-400">{stockHealthStats.lowStock}</span>
                  <span className="text-[11px] text-amber-800/80 dark:text-amber-300 font-medium">Low Stock ({stockHealthStats.lowStockPct}%)</span>
                </div>
                <div className="rounded-lg bg-rose-50 dark:bg-rose-950/30 p-2.5 border border-rose-200 dark:border-rose-800/40">
                  <span className="block font-extrabold text-sm text-rose-700 dark:text-rose-400">{stockHealthStats.outOfStock}</span>
                  <span className="text-[11px] text-rose-800/80 dark:text-rose-300 font-medium">Depleted ({stockHealthStats.outOfStockPct}%)</span>
                </div>
              </div>
            </div>

            {/* CRITICAL ITEMS LIST OR HEALTH STATUS BANNER */}
            {lowStockAlerts.length > 0 ? (
              <div className="space-y-2.5">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {lowStockAlerts.slice(0, 6).map((item) => (
                    <div 
                      key={item.chem.id} 
                      className="flex items-center justify-between p-3.5 rounded-xl border border-rose-200 bg-rose-50/60 dark:border-rose-900/40 dark:bg-rose-950/20"
                    >
                      <div>
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/70 p-4 text-center dark:border-emerald-900/40 dark:bg-emerald-950/20 flex items-center justify-center gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-900/50 dark:text-emerald-400">
        ...
        {/* SECTION 2: RECENT REQUISITION MOVEMENT */}
        <Card title="📜 Recent Requisition Movement" subtitle="Latest store-to-lab chemical transfers & status">
          <div className="space-y-4">
                  ...
                  >
                    <div className="space-y-0.5 min-w-0 pr-2">
                      <div className="flex items-center gap-2">
                        <p className="font-bold text-sm text-[#2e3d19] dark:text-[#eef4e8] truncate" title={entry.chemicalName}>
                          {entry.chemicalName}
                        </p>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-[#eef4e4] text-[#556b2f] dark:bg-[#28301f] dark:text-[#c5d0b5] shrink-0">
                          {entry.lab}
                        </span>
                      </div>
                      <p className="text-xs text-[#71805a] dark:text-[#c5d0b5] truncate">
                        Qty: {entry.qtyRequested || entry.qtyRequestedBase} {entry.baseUnit || 'mL'} • {new Date(entry.date).toLocaleDateString()}
                      </p>
                    </div>
            ...
            {history.length === 0 && (
              <div className="py-8 text-center text-xs text-[#87996c]">
